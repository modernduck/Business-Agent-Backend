...
class Woocommerce extends WordpressClient {
    ...
    public function deleteCategory($name, $parent_id = null)
    {
        $category = $this->getCategory($name, $parent_id);
        $term_id = $category['term_id'];
        return $this->deleteTerm($term_id, self::PRODUCT_CATEGORY);
    }

    public function changeCategoryName($oldName, $newName)
    {
        $oldcategory = $this->getCategory($oldName);
        return $this->editTerm($oldcategory['term_id'], self::PRODUCT_CATEGORY, array(
            "name" => $newName
        ));
